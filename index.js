...
function checkIfIncludes(mainStr, subStr) {
  const sentece = mainStr;
  const word = subStr;
  return sentece.includes(word);
...
function findIndexOf(mainStr, subStr) {
  const sentece = mainStr;
  const word = subStr;
  return sentece.indexOf(word); // Використовуємо метод indexOf(), щоб знайти індекс підрядка subStr в рядку mainStr
}
...
function findLastIndexOf(mainStr, subStr) {
  const sentece = mainStr;
  const word = subStr;
  return sentece.lastIndexOf(word);
